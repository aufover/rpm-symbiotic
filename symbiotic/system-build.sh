...
if [ $FROM -le 7 ]; then
	...
	SYMBIOTIC_VERSION=`cat git_rev-parse.ver`
	cd dg || exit 1
	DG_VERSION=`cat git_rev-parse.ver`
	cd -
	cd sbt-slicer || exit 1
	SBT_SLICER_VERSION=`cat git_rev-parse.ver`
	cd -
	cd sbt-instrumentation || exit 1
	INSTRUMENTATION_VERSION=`cat git_rev-parse.ver`
	cd -

	cd klee || exit 1
	KLEE_VERSION=`cat git_rev-parse.ver`
	cd -
